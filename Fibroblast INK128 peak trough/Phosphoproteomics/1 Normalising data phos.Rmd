...
data_start <- read.delim("1397695209_Aiwei_TMT16_urea_phos_m10.txt", na = c("NaN"), stringsAsFactors = F)
...
master_list <- cbind(data_uniprot$Phosphosite, data_uniprot$uniprot, data_uniprot$main_gene, data_uniprot$Protein.names, data_uniprot$Sequence.window, data_uniprot$Amino.acid, data_uniprot$Position,  data_sl2)


colnames(master_list)[1:7]<- c("Phosphosite", "uniprot", "main_gene", "protein_name", "Sequence.window", "AA", "Position")

write.csv(master_list, "All_normalised_data_phospho.csv", row.names = F)
```
